...
Process {
	...
	Try{
      ...
      if($Env:RD_CONFIG_FORCE -eq "true"){
          $Params.add("DeleteBucketContent", $True) 
      }


      $uri=[System.Uri]$S3Uri

		  Remove-S3Bucket -BucketName $uri.Authority -Force @Params

      Write-Host "Bucket removed"
